...
class OpenExiLibraryImpl extends RealExiLibraryImpl {
  ...
  override def encode(inputStream: BufferedInputStream, outputStream: BufferedOutputStream, settings: Settings): Unit = {
    ...
    // TODO A Test with strict mode (encode)
    settings.fidelityOptionMode match {
  ...
  private def read(inputStream: BufferedInputStream, outputStream: BufferedOutputStream, settings: Settings, validate: Boolean): Unit = {
    val grammarCache = createGrammarCache(settings)
    ...
    settings.fidelityOptionMode match {
      case Strict | Customized =>
        // TODO A Test with strict mode (decode/validate))
  ...
  override def validate(inputFile: ExiFile, settings: Settings): ValidateResult = {
    settings.xsdFile match {
      case Some(_) =>
        ...
        io(inputFile, sinkDevice)(read(_, _, settings, validate = true)) match {
          case Success(_) =>
